...
app = Flask(__name__)
# Ρύθμιση CORS για να δέχεται αιτήματα από το frontend σου
CORS(app)

# --- ΣΤΟΙΧΕΙΑ ΣΥΝΔΕΣΗΣ MONGODB ---
raw_user = "forchatgptpremiumonly_db_user"
raw_pass = "Stelios2026"  # Βεβαιώσου ότι αυτό είναι το τρέχον password στο MongoDB Atlas
cluster_url = "cluster0.6tyqxdb.mongodb.net"
db_name = "quantix_database"

# Ασφαλής κωδικοποίηση χαρακτήρων για το URL
...
try:
    # Προσθήκη tlsAllowInvalidCertificates για συμβατότητα με το Render
...
# --- ENDPOINTS ΓΙΑ ΤΟ SITE (FRONTEND) ---

# Προσθήκη HEAD για να μην βγάζει 405 το cron-job.org
@app.route("/api/health", methods=["GET", "HEAD"])
def health():
    return jsonify({"success": True, "status": "Stable", "database": "Connected"}), 200
...
def register():
    ...
    k = str(data.get("key", "")).upper().strip()
    ...
    if key_data.get("used") == 1:
        return jsonify({"success": False, "error": "This key has already been used"}), 400
    
    if key_data.get("expires_at") < int(time.time()):
        return jsonify({"success": False, "error": "This key has expired"}), 400

    # Έλεγχος αν το Discord ID έχει ήδη λογαριασμό (Περιορισμός 1 Account)
...
def login():
    ...
    if user and check_password_hash(user["password_hash"], p):
        ...
        if not key or key["expires_at"] < int(time.time()):
            return jsonify({"success": False, "error": "License expired"}), 403
        return jsonify({"success": True, "user": {"username": u, "license_key": user["license_key"]}}), 200
    
    return jsonify({"success": False, "error": "Invalid username or password"}), 401

# --- ENDPOINTS ΓΙΑ ΤΟ DISCORD BOT ---
...
def bot_delete_key():
    ...
    if not ok: return err
    data = request.get_json()
    keys_col.delete_one({"license_key": data.get("license_key")})
...
def delete_user_by_id(user_id):
    ok, err = require_bot_auth()
    if not ok: return err
    # Διαγράφουμε όλους τους λογαριασμούς που συνδέονται με αυτό το Discord ID
